...
每个作品必须有一个类型标签（第一个标签）。当前平台只保留两种类型：

| 类型 | 说明 | 适合 |
|------|------|------|
| 交互演示 | 通过固定动态演示 window 展示的单屏或准单屏互动作品 | Canvas/SVG 动画、数据可视化、概念模拟、交互实验 |
| 动态书 | 像网页一样上下滚动浏览的 HTML 作品，内部可包含文字、图表和动态演示 | 长文教程、章节叙事、学习路径、互动讲义 |
...
- **作品类型选择**：根据内容特性选择最合适的作品类型：
  - 单屏动态过程、数据关系、模拟实验 → 交互演示
  - 多章节讲解、长文教程、流程路径、内嵌演示 → 动态书
...
- [ ] title 非空且 ≤ 200 字符
- [ ] tags 不超过 5 个，第一个为作品类型标签（交互演示 / 动态书）
